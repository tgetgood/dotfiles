...
local function worker(args)
    ...
    function bat.update()
        ...
        bat_now.perc = first_line("/sys/class/power_supply/BAT0/capacity")
        bat_now.status = first_line("/sys/class/power_supply/BAT0/status")
        ...
        async("acpi", function(s)
                 local chunks = {}
                 for substring in s:gmatch("%S+") do
                    table.insert(chunks, substring)
                 end
                 bat_now.time = chunks[5]
